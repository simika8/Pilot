...
public static class RandomProduct
{
    ...
    public static List<DemoProduct> Generate(string keyword, string? cursor, int take)
    {
        var res = new List<DemoProduct>();
    ...
    public static DemoProduct GenerateProduct(int productNumber, int lastProductNumber)
    {
        var rnd = new Random(productNumber);
        var id = TestGuid("Product" + productNumber.ToString());
        var name = RandomProduct.NumberToProductName(productNumber, 1, lastProductNumber);
        var p = new DemoProduct()
        {
            //Id = Guid.NewGuid(),
            Id = id,
            Active = rnd.Next(2) == 0,
        ...
        if (IsPrime(productNumber) && p.Type == DemoProductType.Product)
        {
            ...
            {
                p.Stocks.Add(new DemoInventoryStock()
                {
                    Id = Guid.NewGuid(),
                    ProductId = id,
